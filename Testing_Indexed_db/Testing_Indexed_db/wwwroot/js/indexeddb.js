    ...
    async updateEmployee(employeeJson) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                // Ensure employee has all required fields and valid ID
                if (employee.id === undefined || employee.id === null || (typeof employee.id === 'number' && employee.id < 0)) {
                    reject(new Error('Employee must have a valid ID to update'));
                    return;
                }
